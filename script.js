...
loc.addEventListener('click',()=>{
  if(navigator.geolocation){
   ...
   loc.innerHTML="Your browser not Supoorting location accesss";
  }
});
function onSuccess(position){
  let{latitude, longitude}=position.coords;
 
  fetch(' https://api.opencagedata.com/geocode/v1/json?q=${latitude}+${longitude}&key=89c5cc4673b14f0e98df2d9b77104a61')
  .then(response=>response.json()).then(result=>{
    let details= result.results[0].components;
    let {state, postcode, country}=details;
   loc.innerText=`${state}, ${postcode}, ${country}`;
  })
}
function onError(error){
  if(error.code==1){
    loc.innerText="You denied the request";
  }
  else if(error.code==2){
    loc.innerText="Location not available";
  }
  else {
    loc.innerText="Something went wrong";
  }
